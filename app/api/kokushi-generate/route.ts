...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 必要量が減っていく。
    const bankScope = admin.from('exam_question_bank').select('*').eq('mode', 'kokushi').eq('dept', kokushiType);
    ...
    if (freshRaw.length > 0) {
      ...
      if (bankInsertError) console.error('[kokushi-generate] bank insert failed:', bankInsertError);
    }
