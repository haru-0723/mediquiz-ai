...
const questions = [
  {
    ...
    subject: '循環器系',
    difficulty: '標準',
    question: '心臓の左心室から収縮期に血液を全身へ送り出す血管はどれか。',
    options: ['A. 大静脈', 'B. 大動脈', 'C. 肺動脈', 'D. 肺静脈'],
    answer: 'B',
    explanation: '左心室は体循環の起点で、収縮時に大動脈弁を通じて大動脈へ血液を送り出します。大静脈は全身から右心房へ血液を戻す静脈、肺動脈は右心室から肺へ向かう血管です。',
  },
  {
    id: 2,
    subject: '循環器系',
    difficulty: '基礎',
    question: '心電図のP波が表すのはどれか。',
    options: ['A. 心房の興奮', 'B. 心室の興奮', 'C. 心房の再分極', 'D. 心室の再分極'],
    answer: 'A',
    explanation: 'P波は洞結節からの電気信号が心房に伝わり、心房が興奮（脱分極）する様子を表します。QRS波が心室の興奮、T波が心室の再分極を示します。',
  },
  {
    id: 3,
    subject: '呼吸器系',
    difficulty: '基礎',
    question: 'ガス交換が行われる部位はどれか。',
    options: ['A. 気管', 'B. 気管支', 'C. 肺胞', 'D. 気管支細動脈'],
    answer: 'C',
    explanation: '肺胞は薄い一層の上皮細胞でできており、その周囲を毛細血管が取り囲んでいます。この構造により酸素と二酸化炭素のガス交換が効率的に行われます。',
  },
  {
    id: 4,
    subject: '薬理学',
    difficulty: '基礎',
    question: 'アセトアミノフェンの主な作用はどれか。',
    options: ['A. 解熱・鎮痛', 'B. 抗菌', 'C. 降圧', 'D. 利尿'],
    answer: 'A',
    explanation: 'アセトアミノフェンは中枢性の解熱鎮痛薬で、NSAIDsと異なり抗炎症作用は弱いですが、副作用が少なく小児にも使用できます。',
  },
  {
    id: 5,
    subject: '血液凝固',
    difficulty: '標準',
    question: '血液凝固に関与するビタミンはどれか。',
    options: ['A. ビタミンA', 'B. ビタミンC', 'C. ビタミンD', 'D. ビタミンK'],
    answer: 'D',
    explanation: 'ビタミンKは肝臓での凝固因子の合成に必要です。ワルファリンはビタミンKの作用を阻害する抗凝固薬です。',
  },
];
...
export default function QuizPage() {
  ...
  if (phase === 'result') {
    ...
    const finalAccuracy = Math.round((finalCorrect / results.length) * 100);
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-8">
        <div className="bg-white rounded-2xl border p-8 max-w-md w-full text-center">
          ...
          <p className="text-5xl font-bold text-green-600 mb-2">{finalAccuracy}%</p>
          <p className="text-gray-500 mb-8">{results.length}問中 {finalCorrect}問正解</p>
          <div className="flex gap-3">
            <Link href="/dashboard" className="flex-1 border border-gray-200 rounded-xl py-3 text-sm text-gray-600 hover:border-gray-300 text-center">
              ダッシュボードへ
            </Link>
            <button onClick={() => { setCurrent(0); setSelected(null); setAnswered(false); setResults([]); setPhase('quiz'); }}
          ...
          <div className="space-y-3">
            {q.options.map(opt => {
              ...
              if (!answered) {
                ...
                className += 'border-gray-100 opacity-60';
              }
                ...
                <div key={opt} className={className} onClick={() => handleAnswer(opt)}>
                  <div className={`w-6 h-6 rounded-full border flex items-center justify-center text-xs font-medium flex-shrink-0
                    ${answered && isCorrect ? 'bg-green-600 border-green-600 text-white' :
                      answered && isSelected ? 'bg-red-400 border-red-400 text-white' :
