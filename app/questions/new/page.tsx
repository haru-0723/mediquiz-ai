    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-8">
        <div className="bg-white rounded-2xl border border-slate-200 p-8 max-w-md w-full text-center">
          ...
          <h2 className="text-xl font-semibold text-slate-900 mb-2">問題を追加しました！</h2>
          <div className="flex gap-3 mt-6">
            <button onClick={() => { setQuestion(''); setOptionA(''); setOptionB(''); setOptionC(''); setOptionD(''); setExplanation(''); setDone(false); }}
              className="flex-1 border border-slate-200 rounded-xl py-3 text-sm text-slate-600">
              続けて追加
            </button>
            <Link href="/questions" className="flex-1 bg-emerald-600 text-white rounded-xl py-3 text-sm font-medium text-center">
              問題一覧へ
            </Link>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/80 backdrop-blur">
        <nav className="mx-auto flex h-14 max-w-xl items-center justify-between px-8">
          <Logo href="/dashboard" />
          <Link href="/dashboard" className="text-sm text-slate-500">ダッシュボードへ戻る</Link>
        </nav>
      </header>

      <div className="max-w-xl mx-auto p-8">
        <h1 className="text-2xl font-semibold text-slate-900 mb-2">問題を追加</h1>
        <p className="text-slate-500 text-sm mb-8">4択問題を手動で作成できます。</p>
