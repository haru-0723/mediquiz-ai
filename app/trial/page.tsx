    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-md mx-auto p-8 pt-16 text-center">
          ...
          <h1 className="text-2xl font-semibold text-gray-900 mb-2">お試し問題、お疲れさまでした！</h1>
          <p className="text-5xl font-bold text-green-600 mb-2">{accuracy}%</p>
          <p className="text-gray-500 mb-8">{SAMPLE_QUESTIONS.length}問中 {correctCount}問正解</p>
          <div className="bg-white rounded-2xl border p-6 mb-4 text-left">
            <p className="text-sm font-semibold text-gray-900 mb-2">✨ 次は自分の教材で問題を作ってみましょう</p>
          ...
          <div className="space-y-3">
            <Link href="/upload"
              className="block w-full bg-green-600 text-white rounded-xl py-3 text-sm font-medium hover:bg-green-700 transition-colors">
              📤 自分の教材をアップロードする
            </Link>
            <Link href="/dashboard"
              className="block w-full border border-gray-200 rounded-xl py-3 text-sm text-gray-600 hover:border-gray-300 transition-colors">
              ダッシュボードへ戻る
            </Link>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto p-4 sm:p-8">
        <div className="mb-4">
          <p className="text-xs text-green-600 font-medium mb-1">🎯 お試し問題（教材アップロード不要）</p>
          <div className="flex justify-between items-center text-sm text-gray-500">
            <span>{current + 1} / {SAMPLE_QUESTIONS.length}問</span>
          </div>
        </div>
        <div className="h-2 bg-gray-200 rounded-full mb-6 overflow-hidden">
          <div className="h-full bg-green-600 rounded-full transition-all" style={{ width: `${(current / SAMPLE_QUESTIONS.length) * 100}%` }} />
        </div>
        <div className="bg-white rounded-2xl border p-6 mb-4">
          <div className="flex gap-2 mb-4">
            ...
            <span className="bg-gray-100 text-gray-500 text-xs px-3 py-1 rounded-full">{DIFF_LABEL[q.difficulty]}</span>
          </div>
